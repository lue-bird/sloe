...
macro_rules! origin_new {
    ($variable_name:ident, $type_name:ident) => {
        struct $type_name();
        let $variable_name = unsafe { $crate::sloe::origin_new_use_macro_instead($type_name()) };
    };
}
pub use origin_new;
...
/// a stack
pub type Linked_list<Originø, Elementø> = Cons·Empty<
    Head·tail<Elementø, Slot<Originø>>,
    Blank,
>;
pub fn example(Blank {}: Blank) -> U32 {
    3u32
}
pub fn linked_list_vacate_from_backing<
    Elements_backingø: 'static,
    Originø: 'static,
    Elementø: 'static,
>(
    Backing·elements_backing·linked_list·vacate_element {
        backing: backing,
        linked_list: linked_list,
        elements_backing: elements_backing,
        vacate_element: vacate_element,
    }: Backing·elements_backing·linked_list·vacate_element<
        Vec<Originø, Cons·Empty<Head·tail<Elementø, Slot<Originø>>, Blank>>,
        Elements_backingø,
        Cons·Empty<Head·tail<Elementø, Slot<Originø>>, Blank>,
        Fnø_<Backing·element<Elements_backingø, Elementø>, Elements_backingø>,
    >,
) -> Backing·elements_backing<
    Vec<Originø, Cons·Empty<Head·tail<Elementø, Slot<Originø>>, Blank>>,
    Elements_backingø,
> {
    match linked_list {
        Cons·Empty::Empty(Blank {}) => {
            Backing·elements_backing {
                backing: backing,
                elements_backing: elements_backing,
            }
        }
        Cons·Empty::Cons(Head·tail { head: head, tail: tail_slot }) => {
            Backing·elements_backing {
                backing: backing,
                elements_backing: elements_backing,
            }
...
pub fn linked_list_pop<Elementø: 'static, Originø: 'static>(
    Backing·linked_list {
        backing: backing,
        linked_list: linked_list,
    }: Backing·linked_list<
        Vec<Originø, Elementø>,
        Cons·Empty<Head·tail<Elementø, Slot<Originø>>, Blank>,
    >,
) -> Backing·linked_list<Vec<Originø, Elementø>, Blank> {
    match linked_list {
        Cons·Empty::Empty(Blank {}) => {
            Backing·linked_list {
                backing: backing,
                linked_list: Cons·Empty::Empty(Blank {}),
            }
        }
        Cons·Empty::Cons(Head·tail { head: head, tail: tail_slot }) => {
            Backing·linked_list {
...
pub fn linked_list_cons<Elementø: 'static, Originø: 'static>(
    Backing·new_head·tail {
        backing: backing,
        tail: tail,
        new_head: new_head,
    }: Backing·new_head·tail<
    ...
    Head·tail<Elementø, Slot<Originø>>,
> {
    match vec_add(New·vec {
        vec: backing,
        new: tail,
    }) {
        Slot·vec { vec: backing, slot: tail_slot } => {
            Backing·linked_list {
                backing: backing,
                linked_list: Cons·Empty::Cons(Head·tail {
                    head: new_head,
                    tail: tail_slot,
                }),
            }
...
pub struct New·vec<Newø, Vecø> {
    pub new: Newø,
    pub vec: Vecø,
}
#[derive(Copy, Clone, Debug)]
pub struct Backing·linked_list<Backingø, Linked_listø> {
    pub backing: Backingø,
    pub linked_list: Linked_listø,
}
#[derive(Copy, Clone, Debug)]
pub struct Slot·vec<Slotø, Vecø> {
    pub slot: Slotø,
    pub vec: Vecø,
}
#[derive(Copy, Clone, Debug)]
pub struct Backing·elements_backing·linked_list·vacate_element<
    Backingø,
    Elements_backingø,
    Linked_listø,
    Vacate_elementø,
> {
    pub backing: Backingø,
    pub elements_backing: Elements_backingø,
    pub linked_list: Linked_listø,
    pub vacate_element: Vacate_elementø,
}
#[derive(Copy, Clone, Debug)]
pub struct Backing·new_head·tail<Backingø, New_headø, Tailø> {
    pub backing: Backingø,
    pub new_head: New_headø,
    pub tail: Tailø,
}
#[derive(Copy, Clone, Debug)]
pub struct Head·tail<Headø, Tailø> {
    pub head: Headø,
    pub tail: Tailø,
}
#[derive(Copy, Clone, Debug)]
pub struct Backing·elements_backing<Backingø, Elements_backingø> {
    pub backing: Backingø,
    pub elements_backing: Elements_backingø,
}
#[derive(Copy, Clone, Debug)]
pub struct Backing·element<Backingø, Elementø> {
    pub backing: Backingø,
    pub element: Elementø,
}
#[derive(Copy, Clone, Debug)]
pub enum Cons·Empty<Consø, Emptyø> {
    Cons(Consø),
    Empty(Emptyø),
}
